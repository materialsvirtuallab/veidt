...
class EnsembleRank(object):
    ...
    def __init__(self, spect_col_df, spect_column, target_spect,
        ...
        Args:
            spect_col_df (pandas.DataFrame): A pandas dataframe object that
                contains at least two columns, one column with unique label,
                another column corresponding to computational spectra
            spect_column (string): The column in the dataframe that contains
                all reference spectra the target spectrum needs to compare with
            target_spect (Nx2 array): The target spectrum to be identify
            label_column (string): The column contains unique label/key of each
                reference spectrum. Used for generating rankings
        """
    ...
    def borda_rank_vote(self, ensemble_pair):
        """
        Use the Borda count method, the Borda count for a spectrum is the sum
        ...
        Args:
            ensemble_pair list(list): A list of preprocessing-similarity pair.
                Each preprocessing-similarity pair with format:
                ...
                spectrum will be computed using similarity_metric in list[1].
        """

        label_vote_pool = defaultdict(int)

        for proc_comb in ensemble_pair:
            copy_simple_ensem = SimpleEnsemble(
                self.target_spect, self.dataframe[self.spect_column].tolist())
        ...
        sorted_borda_rank = sorted(label_vote_pool.items(),
                                   key=operator.itemgetter(1), reverse=True)
        self.borda_rank = pd.DataFrame.from_records(
            sorted_borda_rank, columns=[self.label_col, 'borda_rank'])
    ...
    def calculate_softmax_prob(self, shift_penalty_alpha=0.01, prob_sort=True):
        ...
        self.dataframe['abs_shift'] = np.abs(
            self.dataframe['energy_shift'] -
            self.dataframe['energy_shift'].mean())
        self.dataframe['neg_shift_alpha'] = np.exp(
            np.negative(shift_penalty_alpha * self.dataframe['abs_shift'])
            / (self.dataframe['energy_shift'].std()))
...
class SimpleEnsemble(object):
    """
    Simple ensemble classifier object used for spectrum shift, preprocessing
    ...
    def preprocess_similarity_compute(self, preprocess, similarity_metric):
        """
        For each row, compute the similarity measure between the target and
        reference spectra.
        ...
        self.spect_df['Similarity'] = self.spect_df['Spectra_Simi_obj'].apply(
            lambda x: x.get_shifted_similarity(similarity_metric,
                                               spect_preprocess=preprocess))
